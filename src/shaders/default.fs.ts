...
void main()
{
    ...
    if (dirLight.enabled)
        result += CalcDirLight(dirLight, norm, viewDir, ao);

    // phase 2: Point lights
    for(int i = 0; i < NR_POINT_LIGHTS; i++) {
        if (pointLights[i].enabled)
            result += CalcPointLight(pointLights[i], norm, FragPos, viewDir, ao);
    }
    
    // phase 3: Spot light
    if (spotLight.enabled)
        result += CalcSpotLight(spotLight, norm, FragPos, viewDir, ao);
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir, float ao)
{
    ...
    // combine results
    vec3 ambient = light.ambient * (texture(material.diffuse, TexCoords).rgb + material.color) * ao;
